...
$$\begin{align*}
	...
	&= \frac{c x^{c-1}y^d}{d x^c y^{d-1}} && \text{Differentiating }u \text{ w.r.t }x \text{ on top and }y \text{ on bottom}\\
	&=\frac{c}{d} x^{(c-1)-c}y^{d-(d-1)} && \text{Using exponent rules for division}\\
	&= \frac{c}{d} x^{-1}y^{1} && \text{Simplifying exponents}\\
	&=\frac{c}{d} \frac{y}{x} && \text{Using exponent rules for negative exponents}\\ \end{align*}$$

Using logarithms, we can take a positive monotonic transformation of $u$ 
...
$$\begin{align*}
	MRS_{x, y}	&=\frac{MU_{x}}{MU_{y}} && \text{The definition of MRS}\\
	&= \cfrac{\big(\frac{\partial }{\partial x}\big)}{\big(\frac{\partial}{\partial y}\big)} && \text{Using the definition of marginal utility as partial derivative}\\
	&= \cfrac{\big(\frac{c}{x}\big)}{\big(\frac{d}{y}\big)} && \text{Differentiating }u \text{ w.r.t }x \text{ on top and }y \text{ on bottom}\\
	&=\frac{c}{d} \frac{y}{x} && \text{Multiplying the righthand side by the reciprocal of the denominator,} \frac{y}{d}\\ \end{align*}$$
