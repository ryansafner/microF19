...
.content-box-green[
.green[**Example**]: Suppose a firm's demand is given by:

$$q=12.5-0.25p$$
]

1. Find the function for the marginal revenue curve.

2. Calculate marginal revenue if the firm produces 6 units, and 7 units.
...
| Elasticity | $MR(q)$ | $R(q)$ |
|------------|---------|--------|
| $|\epsilon|>1$  Elastic | $+$ | Increasing |
| $|\epsilon|=1$ Unit | $0$ | Maximized |
| $|\epsilon|<1$ Inelastic | $-$ | Decreasing |
...
ggplot(data.frame(x=c(0,10)), aes(x=x))+
  ...
  geom_text(aes(x=2.5,y=8.5), color = "blue", label="Elastic", size = 6)+
  geom_text(aes(x=7.5,y=3.5), color = "blue", label="Inelastic", size = 6)+
  geom_text(aes(x=5,y=6), color = "blue", label="Unit Elastic", size = 6)+
    scale_x_continuous(breaks=seq(0,10,1),
                     limits=c(0,10),
                     expand=expand_scale(mult=c(0,0.1)))+
...
- $L=0 \implies$ perfect competition (since $P=MC)$
...
ggplot(data.frame(x=c(0,10)), aes(x=x))+
  ...
  geom_label(x=9,y=2, color = "red", label="MC(q)")+
